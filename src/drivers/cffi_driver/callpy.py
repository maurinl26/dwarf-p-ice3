...
# Create the dictionary mapping ctypes to dtypes
ctype2dtype = {}
...
def asarray(ffi, ptr, shape, **kwargs):
    ...
    if T not in ctype2dtype:
        raise RuntimeError(f"Cannot create an array for element type : {T}")
    
    a = np.frombuffer(
        ffi.buffer(ptr, length * ffi.sizeof(T)), ctype2dtype[T]
    ).reshape(shape, **kwargs)
    
    return a 
    
# Interface functions defined in c builder 
...
# TODO : modify for the call_array function of a sympl component
@ffi.def_extern(error=1)
