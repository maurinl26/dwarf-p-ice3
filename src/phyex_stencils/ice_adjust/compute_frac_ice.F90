...
ELEMENTAL SUBROUTINE COMPUTE_FRAC_ICE (CST_XTT, HFRAC_ICE, NEBN_XTMAXMIX, NEBN_XTMINMIX, PFRAC_ICE, PT, KERR)
  ...
  CASE ('T')
    !using Temperature
    PFRAC_ICE = MAX(0., MIN(1., ((NEBN_XTMAXMIX - PT) / (NEBN_XTMAXMIX - NEBN_XTMINMIX))))      ! freezing interval
  CASE ('O')
    !using Temperature with old formulae
    PFRAC_ICE = MAX(0., MIN(1., ((CST_XTT - PT) / 40.)))      ! freezing interval
  CASE ('N')
    !No ice
    PFRAC_ICE = 0.
  CASE ('S')
