...
def test_thermo(
    gt4py_config, externals, fortran_dims, precision, backend, grid, origin
):
    ...
    Fields_Out = {name: result[i] for i, name in enumerate(Fields_OutNames)}

    logging.info(f"Machine precision {np.finfo(float).eps}")

    for fname in Fields_OutNames:
        logging.info(
            f"{F2Py_Mapping[fname]} :: Mean gt4py      {FloatFieldsIJK[F2Py_Mapping[fname]].mean()}"
        )
        logging.info(
            f"{F2Py_Mapping[fname]} :: Mean fortran    {Fields_Out[fname].mean()}"
        )

    assert_allclose(
        Fields_Out["zt"],
        t_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
    assert_allclose(
        Fields_Out["zlv"],
        lv_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
    assert_allclose(
        Fields_Out["zls"],
        ls_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
    assert_allclose(
        Fields_Out["zcph"],
        cph_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
...
def test_cloud_fraction_1(
    ...
    assert_allclose(
        FieldsOut["pths"],
        ths_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
    assert_allclose(
        FieldsOut["prvs"],
        rvs_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
    assert_allclose(
        FieldsOut["prcs"],
        rcs_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
    assert_allclose(
        FieldsOut["pris"],
        ris_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
...
def test_cloud_fraction_2(
    ...
    assert_allclose(
        pcldfr_out,
        cldfr_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
    assert_allclose(
        phlc_hcf_out,
        hlc_hcf_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
    assert_allclose(
        phlc_hrc_out,
        hlc_hrc_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
    assert_allclose(
        phli_hri_out,
        hli_hri_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
    assert_allclose(
        phli_hcf_out,
        hli_hcf_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
