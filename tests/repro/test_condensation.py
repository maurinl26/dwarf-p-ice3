...
def test_condensation(
    ...
    sigqsat_gt4py = from_array(
        sigqsat, dtype=gt4py_config.dtypes.float, backend=gt4py_config.backend
    )
    ...
    assert_allclose(
        FieldsOut["prv_out"],
        rv_out_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
    assert_allclose(
        FieldsOut["prc_out"],
        rc_out_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
    assert_allclose(
        FieldsOut["pri_out"],
        ri_out_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
    ...
    assert_allclose(
        FieldsOut["zq1"],
        q1_gt4py.reshape(grid.shape[0] * grid.shape[1], grid.shape[2]),
        rtol=1e-6,
    )
...
def test_sigrc_computation(
    ...
    inq1_gt4py = ones(grid.shape, dtype=np.int32, backend=gt4py_config.backend)
    src_1d_gt4py = from_array(SRC_1D, dtype=np.float32, backend=gt4py_config.backend)
